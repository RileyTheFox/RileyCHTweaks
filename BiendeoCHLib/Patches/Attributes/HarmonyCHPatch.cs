	...
	public class HarmonyCHPatch : Attribute {
		...
		public void InitializePatch(Harmony harmony, Type patchType, ManualLogSource logger) {
			...
			if (wrapperField != default) {
				...
				targetMethod = wrapperField.GetCustomAttribute<WrapperMethod>().GetMethodInfo(wrapperType.GetCustomAttribute<Wrapper>().WrappedType, logger);
			...
			HarmonyMethod reversePatchMethod = null;
			foreach (var method in patchType.GetMethods(BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)) {
				...
				if (method.Name == "ReversePatch" || method.GetCustomAttribute<HarmonyCHReversePatch>() != null)
				{
					...
					reversePatchMethod = new HarmonyMethod(method);
				}
			}
